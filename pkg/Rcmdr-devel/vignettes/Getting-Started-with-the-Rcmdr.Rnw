...
\date{Version 2.4-0 (last modified: 2017-01-17)}
\maketitle
...
Commander} (version 2.4-0) is shown below. Most menu items lead to dialog
...
are missing. The menus also include dividers, which are not shown here, and menu items leading to
...
library('Rcmdr')
gm <- function(x) {
  t <- gettext(paste0(x, '...'), domain='R-Rcmdr')
  substr(t, 1, nchar(t) -3)
  }
gt <- function(x) {
  t <- gettext(x, domain='R-Rcmdr')
  t
  }
@
\begin{verbatim}
...
\Sexpr{gt('Models')} 
 ...
 | |- \Sexpr{gt('Component+residual plots')}
 | |- \Sexpr{gt('Added-variable plots')}
 | |- \Sexpr{gt('Influence plot')}
 | |- \Sexpr{gt('Effect plots')}
