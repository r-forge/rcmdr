...
\date{Versión 2.6-1 (última modificación: 2023-04-26)}
...
Este documento describe el uso de \textbf{R Commander} bajo de \textbf{R} para \textbf{Microsoft Windows}. 
...
Información sobre la instalación de \textbf{R Commander} en estas plataformas está disponible en el enlace a las notas de instalación en la página de \textbf{R Commander} \href{http://socserv.socsci.mcmaster.ca/jfox/Misc/Rcmdr/index.html}{socserv.socsci.mcmaster.ca/jfox/Misc /Rcmdr/index.html} o en \href{http://tinyurl.com/Rcmdr}{tinyurl.com/Rcmdr}.
...
Con \textbf{R} corriendo, cárguese el paquete \textbf{Rcmdr} tecleando la instrucción \texttt{library(Rcmdr)} en la consola de R \emph{R Console} para iniciar la interfaz gráfica \textbf{R Commander}. 
El funcionamiento óptimo bajo \textbf{Windows}, de \textbf{R Commander} es bajo la interfaz de documento único (\textquotedblleft SDI\textquotedblright) para \textbf{R}\footnote{La versión de \textbf{R} para \textbf{Windows} corre normalmente en la interfaz de documento múltiple (\textquotedblleft MDI\textquotedblright), el cuál contiene la ventana de la consola \emph{R Console}, la ventana de gráficos \emph{Graphical Device} creada durante la sesión y otras ventanas relacionadas con el proceso de \textbf{R}. 
...
Estas y otras imágenes de pantallas en este documento se han realizado bajo \textbf{Windows 7}; si usa otra versión de \textbf{Windows} (o, por supuesto, otro sistema operativo), entonces la apariencia de las pantallas puede cambiar\footnote{Observe que \textbf{R Commander} necesita algunos paquetes además de los muchos paquetes \textquotedblleft recomendados \textquotedblright\ que son normalmente distribuidos con \textbf{R}. 
El paquete \textbf{Rcmdr}, los paquetes requeridos y muchos otros paquetes de contribuciones están disponibles para descarga en ``Comprehensive R Archive Network" (CRAN) en \href{http://cran.r-project.org/}{http://cran.r-project.org/}. 
...
Normalmente usará los menús y las cajas de diálogo de \textbf{R Commander} para leer, manipular y analizar datos. 
Se puede minimizar sin riesgo la ventana \emph{R Console}.
...
\item[Herramientas] Entradas de menú para la carga de paquetes de \textbf{R} relacionados con el paquete \textbf{Rcmdr} (por ejemplo, para acceder a datos guardados en otro paquete); para cargar paquetes de extensiones de \textbf{Rcmdr} (véase~\citealp{Fox07},~\citealp{FoxCarvalho12} y la sección~\ref{sec-plugins} más adelante);
...
El \textquotedblleft árbol\textquotedblright\ completo del menú de \textbf{R Commander} (versión 2.3-0) se muestra abajo. 
La mayoría de las entradas de menú llevan a un cuadro de diálogo, como se ilustra más tarde en este manual. 
...
Por ejemplo, si el conjunto de datos no contiene factores (variables categóricas), la entrada de menú para las tablas de contingencia estará inactiva\footnote{Algunos opciones de menú no son mostradas en algunas circunstancias.
Por ejemplo, las entradas de menú de \textbf{R Markdown} en el menú \emph{Fichero} se mostrarán únicamente si la pestaña \emph{R Markdown} es activada. 
Los menús también incluyen divisores, que no son mostrados aquí y las entradas de menú que conducen a cuadros de diálogo, como convención, son seguidos de \texttt{...}, que tampoco se muestran.} .
...
El visor de datos puede permanecer abierto mientras se realizan otras operaciones\footnote{El visor de datos, proporcionado por la función \texttt{showData} de David Firth del paquete \textbf{relimp}~\citep{Firth16}, puede ser lento para conjuntos de datos con muchas variables. 
Cuando el número de variables excede el límite fijado (inicialmente puesto a 100), el visor de la instrucción \texttt{View} de \textbf{R}, estéticamente menos agradable, es usado en su lugar para mostrar el conjunto de datos. 
Para usar \texttt{View} independientemente del número de variables, ponga el límite en 0. 
Véase la ayuda de \textbf{R Commander} para más detalles.}.

\item Un botón plano indica el nombre del modelo estadístico activo --- un modelo lineal (como un modelo de regresión lineal), un modelo lineal generalizado, un modelo logit multinomial o un modelo de regresión ordinal\footnote{Los paquetes con extensiones de \textbf{R Commander} (Fox, 2007; Fox and Carvalho, 2012) pueden proporcionar clases de modelos adicionales.}. 
...
Pulsando el botón \emph{Ejecutar}, que está en la parte derecha debajo de la pestaña \emph{R Script} (o, alternativamente, la combinación de teclas \emph{Ctrl-r}\footnote{Esto es, mantener pulsada la tecla \emph{Ctrl} (o \emph{Control}) y simultáneamente pulsar la tecla \emph{r}.}, para \textquotedblleft
...
Si redimendiona \textbf{R Commander}, el ancho de las siguientes salidas de \textbf{R} será automáticamente ajustado al panel \emph{Salida}.

\textbf{R Commander} es altamente configurable: Aquí se ha descrito la configuración estándar. 
Se pueden hacer cambios en la configuración usando el menú \emph{Herramientas}$\longrightarrow$\emph{\ Opciones\ldots}\ o, más extensamente, ajustando las opciones de \textbf{R Commander} en \textbf{R}\footnote{Una entrada de menú que termina en puntos suspensivos lleva a un cuadro de diálogo, éste es el convenio para las interfaces gráficas. 
...
\item Puede importar datos de un archivo de texto plano (\textquotedblleft ascii\textquotedblright) o del portapapeles, o desde Internet desde una URL, de otro paquete estadístico (\textbf{Minitab}, \textbf{SPSS},
\textbf{SAS}, o \textbf{Stata}) o desde una hoja de cálculo \textbf{Excel}.
...
Cuando esto ocurre la instrucción \texttt{read.table} de \textbf{R} interpreta el primer valor de cada línea como el nombre de la línea.
...
En la mayoría de los contextos, \textbf{R Commander} distingue entre variables numéricas y factores, e intentará evitar que haga cosas no razonables, como calcular la media de un factor.
\end{itemize}

Para leer el fichero \texttt{Nations.txt} en \textbf{R}, seleccione \emph{Datos}$\longrightarrow$\emph{Importar datos}$\longrightarrow$\emph{desde un fichero de texto, portapapeles o URL...} del menú de \emph{R Commander}.  Esta operación mostrará un cuadro de diálogo \emph{Leer archivo de texto, portapapeles o URL}, como se muestra en la figura~\ref{fig-2}.
El nombre por defecto del conjunto de datos es \texttt{Dataset}. 
Podemos cambiar el nombre a \texttt{Nations}.
...
La instrucción para leer y visualizar el conjunto de datos \texttt{Nations} (las instrucciones de \textbf{R} \texttt{read.table} y \texttt{showData}) aparecen en la pestaña \emph{R Script} y en el panel \emph{Salida}. 
También, cuando el conjunto de datos se lee y se convierte en el conjunto de datos activo, una nota aparece en el panel \emph{Mensajes}. 
\textbf{R Commander} también usa una instrucción \texttt{library} para cargar el paquete \textbf{relimp}, que fue usado para mostrar el conjunto de datos; aquí, y en general, los paquetes de \textbf{R} son cargados automáticamente por \textbf{R Commander} cuando son necesarios.
...
Como ejemplo de introducción directa de datos, usamos un conjunto de datos muy pequeño del problema~2.44 en~\citet{Moore00}:

\begin{itemize}
\item Seleccione \emph{Datos}$\longrightarrow$\emph{Nuevo conjunto de datos...} del menú de \emph{R Commander}.  Opcionalmente introduzca el nombre para el conjunto de datos, como \texttt{Problema2.44}, en el cuadro de diálogo resultante y pulse en el botón \emph{Aceptar}. 
...
Originalmente, los nombres de las variables son \texttt{var1} y \texttt{var2}, y los valores de los datos son todos \texttt{NA} (es decir, omitidos). 
...
Los valores que contengan blancos deben encerrarse en comillas simples o dobles (por ejemplo, ``\texttt{alguna postsecundaria}", ``\texttt{menos que secundaria}").
...
{fig-4-es}%
\caption{Editor de datos después de la introducción}%
\label{fig-4}%
\end{figure}
...
Si intenta leer datos que no sean un conjunto de datos (data.frame), se mostrará un error en la ventana de mensajes.}. 
...
Una buena interfaz gráfica debe ser ampliamente autoexplicativa: esperamos que una vez vea como trabaja \textbf{R Commander}, tenga pocos problemas usándola, asistido tal vez por los ficheros de ayuda en línea. 
...
Como demostración, el botón \emph{Aceptar} cierra el diálogo y genera una instrucción de \textbf{R}. 
...
En este caso, hemos seleccionado dos variables numéricas para resumir, \texttt{GDP} e \texttt{infant.mortality}.  Pulsando \emph{Aceptar} se obtienen los resultados a continuación en el panel \emph{Salida}:
\begin{verbatim}
...
Esta prestación está disponible únicamente en los sistemas \textbf{Windows}.
Los gráficos tridimensionales de puntos creados con \emph{Gráficos}$\longrightarrow$\emph{Gráfico 3D}$\longrightarrow$\emph{Nube de puntos 3D...} aparece en una ventana especial \emph{RGL device}; así como como los gráficos creados por los modelos estadísticos \citep{Fox03, FoxHong09} vía \emph{Modelos}$\longrightarrow
$\emph{Gráficos}$\longrightarrow$\emph{Gráficos de efecto} aparecen en una ventana gráfica individual.}.
...
Pulsando el botón \emph{Aceptar} se genera la siguiente instrucción, la salida y establece \texttt{LinearModel.1} como el modelo activo, con su nombre mostrado en el botón \emph{Modelo}: 
\begin{verbatim}
...
Para más información sobre \textbf{R Markdown}, seleccione \emph{Ayuda}$\longrightarrow$\emph{Usando R Markdown} del menú de \textbf{R Commander}\footnote{Éste y otros entradas del menú \emph{Ayuda} pueden necesitar una conexión activa a internet.}.
...
No obstante, puede borrar bloques enteros de código, siempre que los bloques siguientes no dependan de ello. Querrá eliminar bloques de código que produzcan salidas no deseadas. 
...
El primer bloque de código que empieza \texttt{```\{r echo=FALSE\}}) establece algunas opciones del software del paquete \textbf{knitr} (Xie, 2013) que son usadas para procesar el texto \textbf{R Markdown} y esto y este bloque no debería ser normalmente modificados.
...
Puede aprovechar la ventaja del marcado simple proporcionado por \textbf{R Markdown}; por ejemplo, texto encerrado entre asteriscos (por ejemplo, \texttt{*esto es importante*}) será puesto en \textit{itálica}. 
...
La pestaña \emph{R Script} proporciona una forma sencilla de edición, introducción y ejecución de instrucciones.  Las instrucciones generadas por \textbf{R Commander} aparecen en la pestaña \emph{Script} y puede introducir y editar las instrucciones en la pestaña más o menos como en un editor. 
...
Por ejemplo, todas las líneas de una instrucción multilínea deben ser enviadas simultáneamente para ejecución.  Para programación seria con \textbf{R}, es preferible utilizar un editor de instrucciones como el proporcionado para \textbf{R} en las versiones de \textbf{Windows} y \textbf{Mac OS X} , o --- incluso mejor --- un editor de programas o un entorno integrado de desarrollo, como por ejemplo, \textbf{RStudio} \href{https://www.rstudio.com/}{www.rstudio.com}\footnote{\textbf{R Commander} corre bajo \textbf{RStudio}, en ese caso la salida y los mensajes de \textbf{R Commander} se dirigen por defecto a la consola de \textbf{R} en \textbf{RStudio}.}.
...
Muchas de esas extensiones están actualmente disponibles en CRAN y pueden ser descargados e instalados de la manera habitual. 
